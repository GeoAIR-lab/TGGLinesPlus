...
def plot_cliques(result_dict, label, node_size=100, node_labels=True, label_size=8, save_fig=False, save_dir="./") -> None:
    ...
    if(save_fig is True):
        if not(os.path.isdir(save_dir)):
            os.mkdir(save_dir)
        # title, save figure
        figtitle = "graph_cliques_{}".format(label)
        plt.savefig(save_dir+figtitle+".pdf", bbox_inches='tight', format='pdf')      
        # close figure to save on memory if saving many figures at once
        plt.close(fig)
    else:
...
def plot_junctions(result_dict, label, node_size=100, node_labels=True, label_size=8, save_fig=False, save_dir="./") -> None:
    ...
    if(save_fig is True):
        if not(os.path.isdir(save_dir)):
            os.mkdir(save_dir)
        # title, save figure
        figtitle = "graph_junctions_{}".format(label)
        plt.savefig(save_dir+figtitle+".pdf", bbox_inches='tight', format='pdf')      
        # close figure to save on memory if saving many figures at once
        plt.close(fig)
    else:
...
def plot_pathseg_endpoints(result_dict, label, node_size=100, node_labels=True, label_size=8, save_fig=False, save_dir="./") -> None:
    ...
    # get nodes that are junctions and non-junctions
    other_locations = [node for node in graph.nodes() if node not in path_seg_endpoints]
    ...
    if(len(path_seg_endpoints) != 0):
        nx.draw_networkx_nodes(graph, pos=node_locations_plotting, nodelist=path_seg_endpoints, node_color=endpoint_color, **node_options)
        # nx.draw_networkx_edges(graph, pos=node_locations_plotting, edgelist=nx.to_edgelist(graph, path_seg_endpoints), edge_color=endpoint_color, **edge_options)
...
def plot_graph_paths(result_dict, label, node_size=100, endpoints=True, node_labels=True, label_size=8, save_fig=False, save_dir="./") -> None:
    ...
    Parameters:
        ...
        endpoints: boolean, whether to plot path_seg_endpoints on top of graph paths to better see end points and junctions
    ...
    endpoints_list = result_dict["path_seg_endpoints"]
    ...
    # only primary junction nodes and terminal nodes
    if(endpoints):
